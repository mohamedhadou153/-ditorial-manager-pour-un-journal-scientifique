...
$d = DB::table('articles')->select(DB::raw('count(id) as count'))->where('etat','traitement')->where('editorId',auth::guard('editor')->user()->email)->where('reviewer1Id','!=', null)->where('rev_active1','NOT LIKE',"%.com%")->orwhere('reviewer2Id','!=', null)->where('etat','traitement')->where('rev_active2','NOT LIKE',"%.com%")->where('editorId',auth::guard('editor')->user()->email)->get();
